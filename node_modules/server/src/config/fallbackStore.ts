...
const wishlists = new Map<string, StoredWishlist>();
...
export function getFallbackProducts(query?: any) {
  ensureSeeded();
...
export function createFallbackProduct(data: any) {
  ensureSeeded();
  const newProduct = {
    _id: randomUUID(),
    title: data.title,
...
export function updateFallbackProduct(id: string, data: any) {
  ensureSeeded();
...
export function createFallbackUser(input: { name: string; email: string; password: string; role?: 'customer' | 'admin' }) {
  ensureSeeded();
  const user = {
    id: randomUUID(),
    name: input.name,
...
export function updateFallbackUserRefreshToken(userId: string, refreshToken?: string) {
  ensureSeeded();
...
export function clearFallbackUserRefreshToken(userId: string) {
  ensureSeeded();
  const user = users.find((entry) => entry.id === userId);
  if (!user) return null;
...
export function getOrCreateFallbackCart(userId: string) {
  ensureSeeded();
  if (!carts.has(userId)) {
...
export function getFallbackCart(userId: string) {
  ensureSeeded();
  return carts.get(userId) || { _id: randomUUID(), user: userId, items: [] };
}

export function updateFallbackCart(userId: string, updater: (cart: StoredCart) => StoredCart) {
  ensureSeeded();
...
export function getOrCreateFallbackWishlist(userId: string) {
  ensureSeeded();
...
export function updateFallbackWishlist(userId: string, updater: (wishlist: StoredWishlist) => StoredWishlist) {
  ensureSeeded();
...
export function createFallbackOrder(input: StoredOrder) {
  ensureSeeded();
  orders.push(input);
  return input;
}

export function getFallbackOrdersForUser(userId: string) {
  ensureSeeded();
...
export function updateFallbackOrderStatus(orderId: string, status: StoredOrder['status']) {
  ensureSeeded();
...
export function getFallbackSalesReports() {
  ...
  const monthlySales = [] as Array<{ label: string; revenue: number; ordersCount: number }>;
